...
public class RightAction extends AbstractInputAction { 
    ...
    public RightAction(Camera3Pcontroller c, SetSpeedAction r, SceneNode tread, SceneNode top, TerrainBlock ter, MyGame game){  
    	target = c.getTarget();
        runAction = r;
        this.tread = tread;
        terrain = ter;
    ...
    public void performAction(float time, Event e){ 
        ...
        Vector3D newY = new Vector3D(0,1,0);
        float dis = moveAmount * time;
        game.setBotRot(dis * 10);
        tread.rotate(dis* 10, newY);
        float treadX = (float) (-1 * dis * Math.sin(Math.toRadians(game.getBotRot())));
        float treadZ = (float) (-1 * dis * Math.cos(Math.toRadians(game.getBotRot())));
